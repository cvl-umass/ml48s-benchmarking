...
def main():
    ...
    best_val_map = {'average_precision': {'full_macro': 0}}
    best_test_map = {'average_precision': {'full_macro': 0}}
...
def validate(args, split, device, model, val_loader, criterion):
    ...
    def run_validate(loader):
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                output = model(images)
                if args.mlab_loss_func == 'LL' and args.ll_mode == 'perm_correction':
                    loss, correction_idx = criterion(output, target, mask)
                else:
                    loss = criterion(output, target, mask)
                loss = loss.mean()
    ...
    gt_presence = torch.stack(gt_presence)
