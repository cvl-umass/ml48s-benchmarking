...
def get_loss_func(loss_name, args, model=None):
    if loss_name == "BCE":
        ...
        return EntropyMaximizationLoss(alpha=args.eml_alpha, beta=args.eml_beta, balance=args.loss_balance)
    elif loss_name == "LL":
        return IgnoreLargeLoss(mode=args.ll_mode, delta=args.ll_delta, balance=args.loss_balance)
    elif loss_name == "LSL":
        return LabelSmoothingLoss(eps=args.lsl_eps, balance=args.loss_balance)
    elif loss_name == "ROLE":
        return ROLELoss(lamb=args.epl_lamb, k=args.epl_k, L=args.num_classes, balance=args.loss_balance)
    elif loss_name == "WNL":
        if args.wnl_gamma is None:
            args.wnl_gamma = 1 / (args.num_classes - 1)
        return WeakNegativesLoss(gamma=args.wnl_gamma, balance=args.loss_balance)
    else:
